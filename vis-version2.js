...
function makePlot(geneX, geneY, studyID)
{
    ...
    $.getJSON("http://yates.webfactional.com/studies/getData", function(d){
	    ...
	    var getKeys = Object.keys(d.data[0].sample);
	    ...
	    */
	    ...
	    getKeys.forEach( function(keyTemp, i){
			...
			{
			    keyTypeObject.keyMax = d3.max(d.data, function(d){return d.sample[keyTypeObject.key]; }); 
			    keyTypeObject.keyMin = d3.min(d.data, function(d){return d.sample[keyTypeObject.key]; });
		    ...
		    else if (keyTypeObject.dataType == "string")
			{
			    ...
			    var first = new colorObject(d.data[0].sample[keyTemp], "#ff0000");
			    //pushes the first id to the array of strings
			    stringTypesCheck.push(d.data[0].sample[keyTemp]);
			    ...
			    d.data.forEach(function(keyValue){
				    {
